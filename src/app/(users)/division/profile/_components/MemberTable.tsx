      ...
      <div className="mt-16 max-w-full">
        <div className="my-6">
          ...
          <div className="flex items-center gap-x-6">
            ...
            <p className={clsx("text-sm sm:text-sm mt-1 md:text-sm lg:text-[16px] xl:text-[16px] text-black md:text-black lg:text-black font-semibold")}>{teamOwner.user.Student?.UserJob?.jobName}</p>
          </div>
        </div>

        <h2 className="text-xl font-semibold">Member</h2>
        {teamOwner.role === "OWNER" && teamOwner.memberId === session.user?.id ? (
            ...
            {teamMember.length > 0 ? (
              teamMember.map((x, i) => (
                <tr key={i} className="flex justify-between items-center">
                  ...
                  <td className="w-full">
                    <p className={clsx("text-sm sm:text-sm md:text-sm lg:text-[16px] xl:text-[16px] text-black md:text-black lg:text-black font-semibold")}>{x.user ? x.user.Student?.UserJob?.jobName : ""}</p>
                  </td>
                  {teamOwner.memberId === session.user?.id ? (
                    ...
                    <td className="flex justify-end space-x-4 w-1/3">
                      <button onClick={() => router.push(`/partner/user/profile/${x.memberId}`)} className="bg-red-500 scale-75 sm:scale-75 lg:scale-100 text-white py-1 px-2 rounded-lg hover:bg-red-600 transition duration-300">
                        View
                      </button>
                    </td>
                  )}
                ...
                (x, i) =>
                  x.status === "PENDING" && (
                    <tr key={i} className="flex justify-between items-center space-y-5">
                      ...
                      <td className="w-1/3">
                        <p className={clsx("text-sm lg:text-[16px] xl:text-[16px] text-black  font-semibold")}>{x.receiver ? x.receiver.Student?.UserJob?.jobName : ""}</p>
                      </td>
                      {x.type === "INVITE" && x.status === "PENDING" ? (
                        ...
                        <>
                          ...
                          ) : session.user?.id === teamOwner.memberId ? (
                            <td className="flex w-1/3 space-x-4">
