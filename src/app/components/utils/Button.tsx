...
export function LinkButton({ className, children, variant, withArrow, href, loading }: LinkButtonProops) {
  ...
  if (baseButton) {
    return (
      <Link href={href} className={`focus:outline-none text-white bg-base hover:bg-red-600 focus:ring-4 focus:ring-red-400 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 ${className} flex w-fit items-center duration-300`}>
        {withArrow && (
