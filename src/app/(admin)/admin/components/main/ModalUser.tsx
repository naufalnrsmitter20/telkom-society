...
import { userPayloadOne } from "@/utils/relationsip";
import { UpdateAdminById } from "@/utils/server-action/userGetServerSession";
import { Role } from "@prisma/client";
import React, { ChangeEvent, Dispatch, SetStateAction, useState } from "react";
import toast from "react-hot-toast";

export default function ModalUser({ setIsOpenModal, data }: { setIsOpenModal: Dispatch<SetStateAction<boolean>>; data?: userPayloadOne | null }) {
  ...
  const HandleSubmit = async (e: ChangeEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const update = await UpdateAdminById(data?.id as string, formData);
      if (update) {
